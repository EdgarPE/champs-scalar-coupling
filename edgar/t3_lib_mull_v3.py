...
def t3_load_data(input_dir):
    ...
    return train, test, structures, mulliken_charges
...
def t3_preprocess_data(train, test, structures, mulliken_charges):
    ...
    # @see: https://www.kaggle.com/todnewman/keras-neural-net-for-champs
...
def map_atom_info_2(df_1, df_2, atom_idx):
...
def get_dist(df):
    ...
    for atom_idx in [0, 1]:
        df = map_atom_info_2(df, df_temp, atom_idx)
...
def add_features_cosine_distance(df):
...
def t3_create_features(train, test):
    ...
    add_features_cosine_distance(train)
    add_features_cosine_distance(test)

    return train, test


def t3_to_parquet(work_dir, train, test, structures, mulliken_charges):
