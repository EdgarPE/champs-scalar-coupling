...
def train_model_regression(X, X_valid2, X_test, y, y_valid2, params, folds, model_type='lgb', eval_metric='mae',
                           columns=None,
    ...
    prediction_valid = np.zeros((len(X_valid2), folds.n_splits))
    prediction_test = np.zeros((len(X_test), folds.n_splits))
    ...
    for fold_n, (train_index, valid_index) in enumerate(folds.split(X)):
        ...
        if model_type == 'lgb':
            ...
            model.fit(X_train, y_train,
                      ...
                      # callbacks=[lgb.reset_parameter(learning_rate=t7_learning_rate_decay)]
                      )
            # callbacks=[lgb.reset_parameter(learning_rate=t7_learning_rate_decay)]
            # categorical_feature=['qcut_subtype_1','qcut_subtype_2']

            y_pred_valid = model.predict(X_valid)
            y_pred_valid2 = model.predict(X_valid2, num_iteration=model.best_iteration_)
        ...
        if eval_metric != 'group_mae':
            ...
            scores2.append(metrics_dict[eval_metric]['sklearn_scoring_function'](y_valid2, y_pred_valid2))
        else:
            scores.append(metrics_dict[eval_metric]['scoring_function'](y_valid, y_pred_valid, X_valid['type']))
            scores2.append(metrics_dict[eval_metric]['scoring_function'](y_valid2, y_pred_valid2, X_valid['type']))

        prediction_valid[:, fold_n] = y_pred_valid2
        prediction_test[:, fold_n] = y_pred_test
    ...
    prediction_valid = np.median(prediction_valid, axis=1)
    prediction_test = np.median(prediction_test, axis=1)

    print('CV mean score: {0:.4f}, std: {1:.4f}.'.format(np.mean(scores), np.std(scores)))
    print('CV mean score2: {0:.4f}, std: {1:.4f}.'.format(np.mean(scores2), np.std(scores2)))

    result_dict['oof'] = oof
    result_dict['prediction_valid'] = prediction_valid
    result_dict['prediction_test'] = prediction_test
...
def t7_load_feature_criskiev(feature_dir, train_, test_, train_selector, test_selector):
...
def t7_load_feature_crane(feature_dir, structures):
    crane = pd.read_parquet(feature_dir + '/crane/crane_structures.parquet')

    return structures.join(crane)


def t7_load_feature_artgor(feature_dir, train_, test_, train_selector, test_selector):
...
def t7_load_feature_giba(feature_dir, train_, test_, train_selector, test_selector):
...
def t7_load_feature_todnewman(feature_dir, train_, test_, train_selector, test_selector):
...
def t7_load_feature_cis_trans(feature_dir, train_, test_, train_selector, test_selector):
...
def t7_load_feature_todnewton(feature_dir, train_, test_, train_selector, test_selector):
    train = pd.read_parquet(feature_dir + '/giba/train_giba.parquet')[train_selector]
    test = pd.read_parquet(feature_dir + '/giba/test_giba.parquet')[test_selector]
...
def t7_merge_yukawa(input_dir, structures):
...
def t7_merge_qm7eigen(input_dir, structures):
    dtype = {
        # 'atom_index': 'int32',
        # 'connectedness': 'int32',
...
def t7_load_feature_edgar(feature_dir, train_, test_, train_selector, test_selector):
...
def t7_load_data_mulliken_oof(work_dir, train, test):
    ...
    mulliken_charges = pd.read_csv(work_dir + '/t7a_mulliken_train.csv', dtype=dtype)
    ...
    mulliken_charges = pd.read_csv(work_dir + '/t7a_mulliken_test.csv', dtype=dtype)
...
def t7_load_data_contributions_oof(work_dir, train, test):
    train_index = train.index
    test_index = test.index
    ...
    oof_contributions = pd.read_csv(work_dir + '/t7b_contributions_train.csv', dtype=dtype)
    ...
    oof_contributions = pd.read_csv(work_dir + '/t7b_contributions_test.csv', dtype=dtype)
...
def t7_merge_structures(train, test, structures):
    train_index = train.index
    test_index = test.index
...
def t7_distance_feature(train, test):
    train_p_0 = train[['x_0', 'y_0', 'z_0']].values
    train_p_1 = train[['x_1', 'y_1', 'z_1']].values
    test_p_0 = test[['x_0', 'y_0', 'z_0']].values
    test_p_1 = test[['x_1', 'y_1', 'z_1']].values
...
def t7_preprocess_data_mulliken(train, mulliken_charges):
...
def t7_preprocess_data_magnetic_st(train, magnetic_st):
...
def t7_prepare_columns(train, test, good_columns_extra=None):
    good_columns = [
        # 'bond_lengths_mean_y',
    ...
    train_index, valid_index = train_test_split(train.index, random_state=SEED, test_size=0.2)

    y = train['scalar_coupling_constant']

    train_ = train.loc[train_index, good_columns]
    valid_ = train.loc[valid_index, good_columns]
    test_ = test[good_columns]
    y_train_ = y[train_index]
    y_valid_ = y[valid_index]

    return train_, valid_, test_, y_train_, y_valid_


def t7_to_parquet(work_dir, train, test, structures, contributions):
    train.to_parquet(f'{work_dir}/t7_train.parquet')
    test.to_parquet(f'{work_dir}/t7_test.parquet')
    structures.to_parquet(f'{work_dir}/t7_structures.parquet')
    contributions.to_parquet(f'{work_dir}/t7_contributions.parquet')


def t7_read_parquet(work_dir):
    train = pd.read_parquet(f'{work_dir}/t7_train.parquet')
    test = pd.read_parquet(f'{work_dir}/t7_test.parquet')
    structures = pd.read_parquet(f'{work_dir}/t7_structures.parquet')
    contributions = pd.read_parquet(f'{work_dir}/t7_contributions.parquet')

    return train, test, structures, contributions


def t7_to_parquet_tt(work_dir, train, test):
    train.to_parquet(f'{work_dir}/t7_train.parquet')
    test.to_parquet(f'{work_dir}/t7_test.parquet')


def t7_read_parquet_tt(work_dir):
    train = pd.read_parquet(f'{work_dir}/t7_train.parquet')
    test = pd.read_parquet(f'{work_dir}/t7_test.parquet')
...
def t7_do_predict(train, valid, test, y, y_valid, type_name, TARGET_WL, PARAMS, N_FOLD, N_ESTIMATORS, SEED, output_dir):
    # for target in TARGET_WL:
    #     train[f'oof_{target}'] = np.nan
    #     valid[f'oof_{target}'] = np.nan
    ...
    for target in TARGET_WL:
        ...
        result_dict_lgb_oof = train_model_regression(X=train, X_valid2=valid, X_test=test, y=y, y_valid2=y_valid,
                                                     params=_PARAMS,
                                                     folds=folds,
        ...
        valid[f'oof_{target}'] = result_dict_lgb_oof['prediction_valid']
        test[f'oof_{target}'] = result_dict_lgb_oof['prediction_test']
...
def t7_learning_rate_010_decay_power_099(current_iter):
    base_learning_rate = 0.1
    lr = base_learning_rate * np.power(.99, current_iter)
    return lr if lr > 1e-3 else 1e-3


def t7_learning_rate_010_decay_power_0995(current_iter):
    base_learning_rate = 0.1
    lr = base_learning_rate * np.power(.995, current_iter)
    return lr if lr > 1e-3 else 1e-3


def t7_learning_rate_005_decay_power_099(current_iter):
    base_learning_rate = 0.05
    lr = base_learning_rate * np.power(.99, current_iter)
    return lr if lr > 1e-3 else 1e-3


def t7_learning_rate_005_decay_power_099931(current_iter):
    base_learning_rate = 0.05
    lr = base_learning_rate * np.power(.99931, current_iter)
    return lr if lr > 1e-3 else 1e-3


def t7_learning_rate_020_decay_power_099965(current_iter):
    base_learning_rate = 0.2
    lr = base_learning_rate * np.power(.99965, current_iter)
    return max(lr, 3e-3)


def t7_learning_rate_decay(current_iter):
...
train_oof, test_oof, _structures, _contributions = t7_load_data(INPUT_DIR)
...
for type_name in TYPE_WL:
    train, test, structures, contributions = t7_load_data(INPUT_DIR)
    ...
    structures = t7_merge_yukawa(INPUT_DIR, structures)
    gc.collect()
    disp_mem_usage()

    structures = t7_merge_qm7eigen(FEATURE_DIR, structures)
    gc.collect()
    disp_mem_usage()

    structures = t7_load_feature_crane(FEATURE_DIR, structures)
    gc.collect()
    disp_mem_usage()

    train, test = t7_merge_structures(train, test, structures)
    del structures
    gc.collect()
    disp_mem_usage()

    t7_distance_feature(train, test)
    gc.collect()
    disp_mem_usage()

    train, test = t7_load_feature_criskiev(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    train, test = t7_load_feature_artgor(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    train, test = t7_load_feature_giba(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    train, test = t7_load_feature_cis_trans(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    train, test = t7_load_feature_todnewman(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    # train, test = t7_load_feature_edgar(FEATURE_DIR, train, test, train_selector, test_selector)
    # gc.collect()
    # disp_mem_usage()

    # train, test = t7_load_data_mulliken_oof(WORK_DIR, train, test)  # Phase 1. OOF data Mulliken charge
    # gc.collect()
    # disp_mem_usage()
    #
    # train, test = t7_load_data_contributions_oof(WORK_DIR, train, test)  # Phase 2. OOF Contributions (fc, sd, pso, dso)
    # gc.collect()
    # disp_mem_usage()
    ...
    train, valid, test, y, y_valid = t7_prepare_columns(train, test, good_columns_extra=extra_cols)

    # pd.set_option('display.max_rows', 500)
    # print(train.describe().T) # Verbose=True
    # print(train.dtypes.T)

    t7_do_predict(train, valid, test, y, y_valid, type_name, TARGET_WL, PARAMS, N_FOLD, N_ESTIMATORS, SEED, OUTPUT_DIR)
    ...
    train_oof.to_csv(f'{OUTPUT_DIR}/t7_train.csv', index=False)
    test_oof.to_csv(f'{OUTPUT_DIR}/t7_test.csv', index=False)

    del train, test
    gc.collect()

test_oof.rename(columns={'oof_scalar_coupling_constant': 'scalar_coupling_constant'}, inplace=True)
test_oof.to_csv(f'{OUTPUT_DIR}/t7_submission.csv', index=False)
