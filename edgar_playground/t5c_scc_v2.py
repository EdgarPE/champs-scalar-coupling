...
def t5_load_feature_criskiev(feature_dir, train_, test_, train_selector, test_selector):
...
def t5_load_feature_artgor(feature_dir, train_, test_, train_selector, test_selector):
...
def t5_load_feature_giba(feature_dir, train_, test_, train_selector, test_selector):
...
def t5_load_feature_edgar(feature_dir, train_, test_, train_selector, test_selector):
...
TYPE_WL = ['1JHN', '2JHN', '3JHN', '2JHH', '3JHH']
...
for type_name in TYPE_WL:
    ...
    train, test = t5_load_feature_criskiev(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    train, test = t5_load_feature_artgor(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()

    train, test = t5_load_feature_giba(FEATURE_DIR, train, test, train_selector, test_selector)
    gc.collect()
    disp_mem_usage()
