...
result_dict_lgb_oof = train_model_regression(X=X, X_test=X_test, y=y_sd, params=params, folds=folds, model_type='lgb', eval_metric='group_mae', plot_feature_importance=False,
                                                      verbose=500, early_stopping_rounds=200, n_estimators=250)

train['oof_sd'] = result_dict_lgb_oof['oof']
test['oof_sd'] = result_dict_lgb_oof['prediction']

train[['id', 'oof_sd']].to_csv(output_dir + '/t0_oof_sd_train.csv', index=False)
test[['id', 'oof_sd']].to_csv(output_dir + '/t0_oof_sd_test.csv', index=False)