...
A feature generálást szét kell bontani, hogy a független forrásból jövő feature-ököt viszonylat fájdalommentesen
lehessen egybe integrálni. Ez CPU és RAM pazarló, utóbbin könnyebb segíteni swap és GC segítségével.

Mindig az eredeti adatokat kell használni inputnak, a kimenet pedig a feldekorált train és test adatok lesznek új 
oszlopokkal.

Megoldandó még: * név ütközés * feature importance * korreláció
...
from edgar_playground.t3_lib_mull_v3 import train_model_regression
from edgar_playground.t3_lib_mull_v3 import t3_load_data
from edgar_playground.t3_lib_mull_v3 import t3_preprocess_data
from edgar_playground.t3_lib_mull_v3 import t3_create_features
from edgar_playground.t3_lib_mull_v3 import t3_prepare_columns
from edgar_playground.t3_lib_mull_v3 import t3_to_parquet
from edgar_playground.t3_lib_mull_v3 import t3_read_parquet
...
train, test = t3_create_features(train, test, structures)
...
# X.to_csv(f'{OUTPUT_DIR}/t3_mull_v3_X.csv', index=False)
# X_test.to_csv(f'{OUTPUT_DIR}/t3_mull_v3_X_test.csv', index=False)
...
pd.concat([mean, median, std], axis=1).to_csv(f'{OUTPUT_DIR}/t3_mull_v3_train.csv', index=True)
...
pd.concat([mean, median, std], axis=1).to_csv(f'{OUTPUT_DIR}/t3_mull_v3_test.csv', index=True)
