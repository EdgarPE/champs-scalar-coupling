...
from edgar_playground.t2_lib import train_model_regression
from edgar_playground.t2_lib import t2_load_data
from edgar_playground.t2_lib import t2_preprocess_data
from edgar_playground.t2_lib import t2_create_features
from edgar_playground.t2_lib import t2_prepare_columns
from edgar_playground.t2_lib import t2_to_parquet
from edgar_playground.t2_lib import t2_read_parquet
from edgar_playground.t2_lib import t2_create_poly_features

##### COPY__PASTE__LIB__END #####


train, test, sub, structures, contributions = t2_load_data(INPUT_DIR)

train, test = t2_preprocess_data(train, test, structures, contributions)
...
# t2_to_parquet(WORK_DIR, train, test, sub, structures, contributions)

# train, test, sub, structures, contributions = t2_read_parquet(WORK_DIR)

X, X_test, y, labels = t2_prepare_columns(train, test)
...
train[['id'] + [f'oof_{c}' for c in TARGET_WL]].to_csv(f'{OUTPUT_DIR}/t2_baseline_train.csv', index=False)
test[['id'] + [f'oof_{c}' for c in TARGET_WL]].to_csv(f'{OUTPUT_DIR}/t2_baseline_test.csv', index=False)
